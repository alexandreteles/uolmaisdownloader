...
class downloader:
	...
	def aria2c(self, metalink_file, concurrency, outputpath, aria2cpath):
		aria2carguments = "-i " + metalink_file + "-d " + outputpath + "-j " + concurrency + "-c"
		try:
			subprocess.call(aria2cpath, aria2carguments, stdout=PIPE).stdout.read()
			return True
		except:
			print "Impossivel iniciar o download. Verifique o caminho para o binario aria2c ou os links providos para download."
			return False